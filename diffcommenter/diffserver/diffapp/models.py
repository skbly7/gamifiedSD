...
class CommitSequence(models.Model):
    ...
    title = models.TextField(default=u'')
...
class Badge(models.Model):
    ...
    def show(self):
        return '<img src="'+self.url+'" width="28px" height="28px" alt="'+self.name+'"/>'
...
class Commit(models.Model):
    """ один коммит """
    commit_sequence = models.ForeignKey(CommitSequence, verbose_name=u'',
        related_name='commits', null=True, blank=True, on_delete=models.CASCADE)
    sha1 = models.TextField()
    head_lines = models.TextField()
...
class Diff(models.Model):
    ...
    head_lines = models.TextField('')  # объединенные через \n
    body_lines = models.TextField('')  # объединенные через \n
